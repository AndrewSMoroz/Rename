...
namespace Rename.Utility
{
    ...
    // This is from stackoverflow.  I added if (!string.IsNullOrEmpty(dlg.SelectedPath)) { SendKeys.SendWait("{RIGHT}"); } and t.Interval = 500;
    // It works, but you need to set t.Interval to a large enough value for it to work on some machines.
    // It may only work on Windows 10.
    // Also, the selected folder comes into view, but as the very last item in the tree view.  Hitting right arrow puts it in the middle.
                            ...
                            if (hwndFolderCtrl != IntPtr.Zero)
                            {
                                ...
                                if (hwndTV != IntPtr.Zero)
                                {
                                    ...
                                    if (item != IntPtr.Zero)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(dlg.SelectedPath)) { SendKeys.SendWait("{RIGHT}"); }
